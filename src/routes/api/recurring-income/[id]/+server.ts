...
import { deleteRecurringIncome, updateRecurringIncome } from '$lib/server/queries/recurring-income';
...
export const PATCH: RequestHandler = async ({ platform, locals, params, request }) => {
	...
	const updated = await updateRecurringIncome(getDb(platform), params.id!, hid, body.data);
	return json(updated);
};
